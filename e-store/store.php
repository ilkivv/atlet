...
$rsStore = CCatalogStore::GetList(
    array('SORT'=>'ASC'),
    array('ACTIVE'=>'Y',/*'ISSUING_CENTER'=>(($_GLOBALS['CURRENT_CITY']['PROPERTIES']['SALEALLOW']['VALUE_XML_ID'] == 'Y')?'N':'Y'),'SHIPPING_CENTER'=>(($_GLOBALS['CURRENT_CITY']['PROPERTIES']['SALEALLOW']['VALUE_XML_ID'] == 'Y')?'N':'Y'),*/'ID'=>$_GLOBALS['CURRENT_CITY']['PROPERTIES']['stories']['VALUE'], '!=ID' => $exclusionOfStoresFromIssuanceInPopup)
    );
                        ...
                        <?php if ($amount[$v['ID']]) {
                            ...
                            <?php foreach ($amount[$v['ID']] as $ak => $av) {
                                ...
                                if ($av['AMOUNT']>0) {
                                    ...
                                    $amountCnt = 1;
                                    ?><li><?=$SKU[$av['PRODUCT_ID']]['CML2_ATTRIBUTES']['VALUE']?> (<?=$av['AMOUNT']?>) - <?= round($priceItem['PRICE'] * (1-($discount/100)), 0);?> р.</li><?php
                                }
                            }
                            if (!$amountCnt) {
